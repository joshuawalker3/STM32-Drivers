...
int main(void)
{
  ...
  while (1)
  {
	aht10_ioctrl(aht10, TEMP_UNIT_METRIC); //switch to metric units
	aht10_read(aht10, &temperature, &humidity); //read temp and humidity

	HAL_Delay(5000);

	aht10_ioctrl(aht10, TEMP_UNIT_IMPERIAL); //switch to imperial units
	aht10_read(aht10, &temperature, &humidity); //read temp and humidity
